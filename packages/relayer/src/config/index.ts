...
function readConfigFile(): Record<string, unknown> {
  ...
  if (!configPathString) {
    logger.warn("CONFIG_PATH is not set, using default path", {
...
export function getChainConfig(chainId: number): ChainConfig {
  ...
  if (
    ...
    logger.warn(
      `Using default fee_receiver_address: ${CONFIG.defaults.fee_receiver_address} for chain id: ${chainId}`,
      { chainId },
    );
  }
  ...
  if (!chainConfig.entrypoint_address && CONFIG.defaults.entrypoint_address) {
    logger.warn(
      `Using default entrypoint_address ${CONFIG.defaults.entrypoint_address} for chain id: ${chainId}`,
      { chainId },
    );
  }
...
export function getFeeReceiverAddress(chainId: number): string {
  ...
  return (
    chainConfig.fee_receiver_address || CONFIG.defaults.fee_receiver_address
  );
}
...
export const FRAXUSD_ADDRESS = getAddress(
  "0xCAcd6fd266aF91b8AeD52aCCc382b4e165586E29",
); // FRAXUSD
export const WOETH_ADDRESS = getAddress(
  "0xDcEe70654261AF21C44c093C300eD3Bb97b78192",
); // WOETH
export const EXCEPTION_TOKENS = [FRAXUSD_ADDRESS, WOETH_ADDRESS];
