...
export class QuoteService {
  ...
  async quoteFeeBPSNative(quoteParams: QuoteFeeBPSParams): Promise<QuoteFee> {
    ...
    const extraGasDetail = extraGas
      ? {
          extraGasTxCost: this.extraGasTxCost,
          extraGasFundAmount: this.extraGasFundAmount,
        }
      : undefined;
    ...
    if (assetAddress.toLowerCase() === NativeAddress.toLowerCase()) {
      ...
      quote = await quoteProvider.quoteNativeTokenInERC20(
        chainId,
        assetAddress,
        amountIn,
      );
    ...
    const feeBPS = await this.netFeeBPSNative(
      baseFeeBPS,
      amountIn,
      quote,
      gasPrice,
