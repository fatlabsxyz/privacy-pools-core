...
function readConfigFile() {
    ...
    return JSON.parse(fs.readFileSync(path.resolve(configPathString), { encoding: "utf-8" }));
}
 ...
 */
export function getFeeReceiverAddress(chainId) {
 ...
 */
export function getSignerPrivateKey(chainId) {
    const chainConfig = getChainConfig(chainId);
 ...
 */
export function getEntrypointAddress(chainId) {
    const chainConfig = getChainConfig(chainId);
 ...
 */
export function getQuoteExpirationTime() {
 ...
 */
export function getAssetConfig(chainId, assetAddress) {
    const chainConfig = getChainConfig(chainId);
    if (!chainConfig.supported_assets) {
        throw RelayerError.assetNotSupported();
    }
    const assetConfig = chainConfig.supported_assets.find(asset => asset.asset_address.toLowerCase() === assetAddress.toLowerCase());
    if (!assetConfig) {
        throw RelayerError.assetNotSupported();
    }
    return assetConfig;
